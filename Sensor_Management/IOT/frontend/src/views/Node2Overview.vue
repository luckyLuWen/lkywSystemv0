...
<template>
  <div class="page-container">
    ...
    <div class="status-banner" :class="{ 'online': store.connected, 'offline': !store.connected }">
      <div class="icon">{{ store.connected ? '🟢' : '🔴' }}</div>
      <div class="info">
        <h2>{{ store.connected ? '系统在线' : '连接中断' }}</h2>
        <p>设备 IP: 192.168.0.241 (五合一 B)</p>
      </div>
    </div>
    ...
    <div class="device-list">
      ...
      <div class="device-item" @click="$router.push('/node2/weather')">
        <div class="device-icon">🌤️</div>
        <div class="device-info">
          <h4>气象监控 (温湿度)</h4>
          <p>数据: {{ store.data.node2.temp }}℃ | {{ store.data.node2.hum }}%</p>
        </div>
        <div class="arrow">详情 -></div>
      </div>

      <div class="device-item" @click="$router.push('/node2/env')">
        <div class="device-icon">☢️</div>
        <div class="device-info">
          <h4>环境监控 (烟雾/TVOC/CO)</h4>
          <p>
            TVOC: {{ store.data.node2.tvoc }} | 
            烟雾: {{ store.data.node2.smoke }} | 
            CO: {{ store.data.node2.co }}
          </p>
        </div>
        <div class="arrow">详情 -></div>
      </div>
