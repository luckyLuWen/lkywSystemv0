...
const nodeState = computed(() => store.nodes.node2)
</script>
    ...
    <div class="device-list">
      <div class="device-item" @click="$router.push('/node2/weather')">
        ...
        <div class="device-info">
          <h4>气象监控</h4>
          <p>数据: {{ store.data.node2.temp }}°C | {{ store.data.node2.hum }}%</p>
        </div>
      ...
      <div class="device-item" @click="$router.push('/node2/env')">
        ...
        <div class="device-info">
          <h4>环境监控</h4>
          <p>TVOC: {{ store.data.node2.tvoc }} | 烟雾: {{ store.data.node2.smoke }} | CO: {{ store.data.node2.co }}</p>
        </div>
        <div class="arrow">详情 →</div>
      </div>
    </div>
