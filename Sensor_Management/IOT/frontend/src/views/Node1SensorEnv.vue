...
const initCharts = () => {
  ...
  if (chartSmokeRef.value) {
    ...
    myChartSmoke.setOption({
      // 🟢 [修改] 标题单位
      title: { text: '🌫️ 烟雾趋势 (ug/m³)', left: 'center', textStyle: { color: '#909399' } },
...
const fetchHistory = async (params = {}) => {
  try {
    ...
    const smokes = data.map(item => item.n1_smoke)
    const tvocs = data.map(item => item.n1_tvoc)
    const cos = data.map(item => item.n1_co)
...
const getStatus = (key, val) => {
  let isDanger = false
  // 🟢 [修改] 烟雾报警阈值改为 600,000 (约 500ppm)
  if (key === 'smoke' && val > 600000) isDanger = true
  if (key === 'tvoc' && val > 2.0) isDanger = true
  if (key === 'co' && val > 50) isDanger = true
  
  // 计算进度条百分比
  let max = 100
  if (key === 'smoke') max = 1000000 // 100万量程
  if (key === 'tvoc') max = 2.0
  if (key === 'co') max = 100
  ...
  return {
    ...
    percent: Math.min((val / max) * 100, 100) + '%'
  }
}
  ...
  <div class="full-page">
    ...
    <div class="header-bar">
      <div class="left">
        ...
        <div class="titles">
          <h2>📍 节点 A - 空气质量监测</h2>
          <span class="subtitle">实时数据 | 历史趋势 | 安全预警</span>
        </div>
      </div>
    ...
    <div class="monitor-row">
      <div 
        ...
        :key="item.key" 
        class="big-card"
        :class="getStatus(item.key, store.data.node1[item.key]).class"
      >
        <div class="card-header">
          <span class="icon">{{ item.icon }}</span>
          <span class="name">{{ item.name }}</span>
          <span class="status-tag" :style="{ color: getStatus(item.key, store.data.node1[item.key]).color }">
            {{ getStatus(item.key, store.data.node1[item.key]).text }}
          </span>
        </div>
        <div class="card-body">
          <div class="value">
            {{ store.data.node1[item.key] }}
            <span class="unit">{{ item.unit }}</span>
          </div>
          <div class="progress-bg">
            <div class="progress-fill" :style="{ width: getStatus(item.key, store.data.node1[item.key]).percent, backgroundColor: getStatus(item.key, store.data.node1[item.key]).color }"></div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="info-section">
      ...
      <table>
        ...
        <tbody>
          ...
          <tr><td>🧪 TVOC</td><td>0-0.5</td><td>0.5-0.8</td><td class="danger-cell">> 2.0</td><td>头痛嗜睡</td></tr>
...
.value { font-size: 2.5rem; font-weight: bold; color: #303133; text-align: center; margin: 10px 0; }
