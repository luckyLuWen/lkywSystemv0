    ...
    <div class="device-list">
      ...
      <div class="device-item" @click="$router.push('/node1/weather')">
        ...
        <div class="device-info">
          <h4>气象监控 (温湿度)</h4>
          <p>数据: {{ store.data.node1.temp }}℃ | {{ store.data.node1.hum }}%</p>
        </div>
      ...
      <div class="device-item" @click="$router.push('/node1/env')">
        ...
        <div class="device-info">
          ...
          <p>
            TVOC: {{ store.data.node1.tvoc }} | 
            烟雾: {{ store.data.node1.smoke }} | 
            CO: {{ store.data.node1.co }}
          </p>
        </div>
        <div class="arrow">详情 -></div>
      </div>
