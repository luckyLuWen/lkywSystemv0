...
const fetchHistory = async (params = {}) => {
  try {
    ...
    // 👇👇👇 这里的字段改为 n2_temp 和 n2_hum 👇👇👇
    const times = data.map(item => item.timestamp.substring(11, 19))
    const temps = data.map(item => item.n2_temp)
    const hums = data.map(item => item.n2_hum)
...
<template>
  <div class="page-container">
    ...
    <div class="realtime-cards">
      <div class="card temp-card"><div class="label">实时温度</div><div class="value">{{ store.data.node2.temp }} <small>℃</small></div></div>
      <div class="card hum-card"><div class="label">实时湿度</div><div class="value">{{ store.data.node2.hum }} <small>%</small></div></div>
    </div>
