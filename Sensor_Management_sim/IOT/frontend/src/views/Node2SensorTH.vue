...
async function fetchHistory(params = {}) {
  try {
    ...
    const temps = rows.map((item) => Number(item.n2_temp ?? 0))
    const hums = rows.map((item) => Number(item.n2_hum ?? 0))

    tempChart?.setOption({ xAxis: { data: times }, series: [{ data: temps }] })
    humChart?.setOption({ xAxis: { data: times }, series: [{ data: hums }] })
  } catch (error) {
    console.error('Node2 weather history error:', error)
  }
}
...
async function refreshSystemStatus() {
  try {
    ...
    console.error('Node2 status error:', error)
  }
}
...
async function toggleSystem() {
  ...
  try {
    ...
    console.error('Node2 control error:', error)
  }
}
  ...
  <div class="page-container">
    ...
    <div class="realtime-grid">
      <div class="metric-card warm">
        ...
        <strong class="metric-value">{{ store.data.node2.temp }}</strong>
        <span class="metric-unit">°C</span>
      </div>
      <div class="metric-card cool">
        <span class="metric-label">实时湿度</span>
        <strong class="metric-value">{{ store.data.node2.hum }}</strong>
