...
const sensors = [
  ...
  { key: 'tvoc', label: 'TVOC', unit: 'mg/m3', field: 'n2_tvoc', color: '#7c3aed', threshold: 2.0 },
  { key: 'co', label: '一氧化碳', unit: 'ppm', field: 'n2_co', color: '#ef4444', threshold: 50 },
]
...
async function fetchHistory(params = {}) {
  try {
    ...
    smokeChart?.setOption({ xAxis: { data: times }, series: [{ data: rows.map((item) => Number(item.n2_smoke ?? 0)) }] })
    tvocChart?.setOption({ xAxis: { data: times }, series: [{ data: rows.map((item) => Number(item.n2_tvoc ?? 0)) }] })
    coChart?.setOption({ xAxis: { data: times }, series: [{ data: rows.map((item) => Number(item.n2_co ?? 0)) }] })
  } catch (error) {
    console.error('Node2 env history error:', error)
  }
}
...
async function refreshSystemStatus() {
  try {
    ...
    console.error('Node2 env status error:', error)
  }
}
...
async function toggleSystem() {
  ...
  try {
    ...
    console.error('Node2 env control error:', error)
  }
}
  ...
  <div class="page-container">
    <div class="header-bar">
      ...
      <div class="header-actions">
        <span class="status-badge" :class="store.nodes.node2.online ? 'online' : 'offline'">
          {{ store.nodes.node2.online ? '节点在线' : '节点离线' }}
        </span>
